...
const lemburUserIdSet = computed(() => new Set((props.lemburUserIds ?? []).map((v) => Number(v))));
const page = usePage();
...
function rowStatus(row) {
    ...
    if (masuk && keluar) {
        return { label: 'Hadir lengkap', tone: 'success' };
    }
    if (masuk && !keluar) {
        return { label: 'Belum checkout', tone: 'warning' };
    }
    return { label: 'Belum presensi', tone: 'danger' };
}
                                ...
                                >
                                    ...
                                    <td class="px-4 py-4 text-sm">
                                        <div class="flex flex-wrap items-center gap-2">
                                            ...
                                            <StatusBadge
                                                v-if="lemburUserIdSet.has(Number(row.user_id))"
                                                label="Lembur"
                                                tone="info"
                                            />
                                        </div>
